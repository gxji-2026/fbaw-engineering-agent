#!/usr/bin/env python3
"""
fbaw_engineering_agent_3Rx4_V4_2_3a_dsh_qualification.py
...
V4.2.4 DeepSeek + DSH qualification upgrade:
...
1) Full Python run using the supplied MBVD seeds, no cloud LLM:
    python fbaw_engineering_agent_3Rx4_V4_2_3a_dsh_qualification.py --no-llm

2) Use COMSOL exports for all three resonators:
    python fbaw_engineering_agent_3Rx4_V4_2_3a_dsh_qualification.py ^
        --comsol-low 6p22.txt ^
...
def _run_command_capture(command, timeout_s: float = 120.0, cwd: Optional[str] = None) -> dict:
    """Run a local argv/string command and capture a compact diagnostic result."""
    ...
    try:
        ...
        return {"command": display, "cwd": cwd, "returncode": None, "stdout": "", "stderr": "", "ok": False, "error": str(exc)}


def _split_command_prefix(command: str) -> list[str]:
    """Split DSH command prefix safely for the local OS without shell re-quoting."""
    return shlex.split(command, posix=(os.name != "nt"))

def _scan_dsh_trace_evidence(trace_root: Optional[str], challenge: str, ack: str) -> dict:
    """Optional conservative trace scan. Never upgrades to PASS without explicit tool/result evidence."""
    if not trace_root:
        return {
            "status": "not_checked",
            "trace_root": None,
            "trace_verified_pass": False,
            "reason": "No --dsh-trace-root supplied; headless stdout is treated as final-answer only.",
        }
    ...
    if not root.is_dir():
        return {
            ...
            "trace_root": str(root),
            "trace_verified_pass": False,
            "reason": "Trace root does not exist or is not a directory.",
        }
    candidates = []
    for pat in ("*.jsonl", "*.json", "*.log", "*.txt"):
        candidates.extend(root.rglob(pat))
    candidates = sorted(candidates, key=lambda x: x.stat().st_mtime if x.exists() else 0, reverse=True)[:200]
    tool_markers = ("tool_result", "tool/result", '"role":"tool"', '"role": "tool"', "tool-call", "tool_call")
    hits = []
    for f in candidates:
        try:
        ...
        has_ack = ack in text
        has_tool_marker = any(m in text for m in tool_markers)
        if has_challenge or has_ack:
            hits.append({
                "file": str(f),
                "challenge_seen": has_challenge,
                "ack_seen": has_ack,
                "tool_marker_seen": has_tool_marker,
            })
        if has_challenge and has_ack and has_tool_marker:
            return {
                "status": "verified",
                "trace_root": str(root),
                "trace_verified_pass": True,
                "evidence_file": str(f),
                "evidence": hits[-1],
            }
    return {
        ...
        "trace_root": str(root),
        "trace_verified_pass": False,
        "evidence_hits": hits[:20],
        "reason": "No single persisted trace file contained challenge + ACK + an explicit tool-call/result marker.",
    }


def run_dsh_harness_diagnostic(
    *,
    dsh_command: str,
    dsh_workdir: Optional[str] = None,
    profile: str = "headless",
    timeout_s: float = 180.0,
    trace_root: Optional[str] = None,
) -> dict:
    """
    V4.2.4 DSH qualification probe.

    Level A -- launcher: DSH boots and reports a version.
    Level B -- black-box harness: a headless task is instructed to execute a shell
               challenge and to emit one exact ACK only after observing the tool
               result. Because headless stdout may expose only the final assistant
               text, PASS does NOT require the intermediate challenge token there.
    Level C -- optional trace verification: conservatively inspect an explicitly
               supplied persisted trace root for challenge + ACK + tool evidence.

    Black-box PASS is sufficient to continue the engineering qualification.
    Trace verification is reported separately and never inferred from stdout.
    """
    prefix = _split_command_prefix(dsh_command)
    resolved_workdir = str(Path(dsh_workdir).expanduser().resolve()) if dsh_workdir else None
    if resolved_workdir and not Path(resolved_workdir).is_dir():
        raise RuntimeError(f"DSH workdir does not exist or is not a directory: {resolved_workdir}")

    version_cmd = prefix + ["--version"]
    version = _run_command_capture(version_cmd, timeout_s=min(60.0, timeout_s), cwd=resolved_workdir)

    py = sys.executable.replace('"', '\\"')
    challenge = 'DSH_TOOL_EXEC_OK_V424'
    ack = 'DSH_HARNESS_OK_V424'
    job = (
        'Use an available shell or terminal tool to execute exactly this local command: '
        f'"{py}" -c "print(\\"{challenge}\\")". '
        f'Inspect the actual tool result. Only if the command output contains {challenge}, '
        f'reply exactly: {ack}. If the tool fails, reply with a failure explanation and DO NOT emit {ack}.'
    )
    headless_cmd = prefix + ["--profile", profile, job]
    headless = _run_command_capture(headless_cmd, timeout_s=timeout_s, cwd=resolved_workdir)

    stdout = (headless.get("stdout") or "").strip()
    stderr = (headless.get("stderr") or "").strip()
    ...
    known_errors = [sig for sig in known_error_signatures if sig in combined_lower]

    ack_exact = stdout == ack
    ack_seen_anywhere = ack in stdout
    nonempty_lines = [line.strip() for line in stdout.splitlines() if line.strip()]
    final_nonempty_line = nonempty_lines[-1] if nonempty_lines else ""
    ack_is_final_line = final_nonempty_line == ack
    # Headless DSH may prepend a short explanation before the requested final answer.
    # Therefore black-box success requires the ACK to be the final non-empty line, not
    # necessarily the entire stdout. We still reject known launcher/sandbox/tool errors.
    # Deliberately DO NOT infer tool execution from challenge-token substring alone.
    black_box_pass = bool(
        version.get("ok")
        and headless.get("ok")
        and ack_is_final_line
        and not known_errors
    )

    trace = _scan_dsh_trace_evidence(trace_root, challenge, ack)
    trace_verified_pass = bool(trace.get("trace_verified_pass"))

    result = {
        "dsh_command": dsh_command,
        "dsh_workdir": resolved_workdir,
        "profile": profile,
        "version_probe": version,
        "headless_tool_probe": headless,
        "challenge_token": challenge,
        "ack_token": ack,
        "ack_exact": ack_exact,
        "ack_seen_anywhere": ack_seen_anywhere,
        "final_nonempty_line": final_nonempty_line,
        "ack_is_final_line": ack_is_final_line,
        "known_error_signatures_seen": known_errors,
        "black_box_harness_pass": black_box_pass,
        "trace_verification": trace,
        "trace_verified_pass": trace_verified_pass,
        "dsh_harness_tool_loop_pass": black_box_pass,
        "qualification_level": (
            "TRACE_VERIFIED" if trace_verified_pass else
            "BLACK_BOX_VERIFIED" if black_box_pass else
            "FAILED"
        ),
        "scope": "DSH launcher + headless conditional tool challenge + final ACK; optional persisted-trace verification",
        "not_proven": (
            "Without trace verification, intermediate tool/call and tool/result events are not directly observed. "
            "FBAW tools are also not yet registered natively in DSH."
        ),
    }

    print("\n" + "="*78)
    print("DSH HARNESS QUALIFICATION / V4.2.4")
    print("="*78)
    print("DSH workdir:", resolved_workdir or os.getcwd())
    print("DSH version probe:", "PASS" if version.get("ok") else "FAIL")
    print("DSH black-box tool-loop probe:", "PASS" if black_box_pass else "FAIL")
    print("DSH persisted-trace verification:", "PASS" if trace_verified_pass else trace.get("status", "NOT CHECKED").upper())
    print("DSH qualification level:", result["qualification_level"])
    if version.get("stdout"):
        print("DSH version output:", version["stdout"].strip())
    if not black_box_pass or trace_root:
        print("DSH diagnostic detail:")
...
def run_engineering_agent_v4_2(
    start,
    low: MBVD,
    high: MBVD,
    f_ghz: np.ndarray,
    ...
    for step in range(1, max_agent_steps + 1):
        ...
        if ds is not None:
            try:
                ...
                decision = None
                parse_error = None
                raw_arg_preview = None
                for json_attempt in range(1, max(1, int(planner_json_retries) + 1) + 1):
                    retry_messages = messages
                    if json_attempt > 1:
                        retry_messages = messages + [{
                            "role": "user",
                            "content": (
                                "Your previous tool-call arguments were not complete valid JSON. "
                                "Retry the same planning step from the unchanged Python-verified state. "
                                "Return exactly one allowed tool call and ensure function.arguments is complete valid JSON."
                            ),
                        }]
                    response = _deepseek_chat_completion(
                        model=planner_model,
                        ...
                        thinking=True,
                        strict_tools=strict_tools,
                        max_tokens=max(1024, int(planner_max_tokens)),
                    )
                    decision, parse_error = _extract_deepseek_tool_call(response)
                    if not parse_error:
                        if json_attempt > 1:
                            print(f"DeepSeek planner JSON recovered on retry {json_attempt-1}/{planner_json_retries}")
                        break
                    msg0 = ((response.get("choices") or [{}])[0].get("message") or {})
                    calls0 = msg0.get("tool_calls") or []
                    if calls0:
                        raw0 = ((calls0[0].get("function") or {}).get("arguments"))
                        raw_arg_preview = repr(raw0)[:600]
                    print(
                        f"DeepSeek malformed tool JSON on attempt {json_attempt}/"
                        f"{max(1, int(planner_json_retries) + 1)}: {parse_error}"
                    )
                    if raw_arg_preview:
                        print("Raw tool arguments preview:", raw_arg_preview)
                if parse_error:
                    raise RuntimeError(
                        f"{parse_error}; exhausted {max(1, int(planner_json_retries) + 1)} DeepSeek attempts "
                        f"with planner_max_tokens={planner_max_tokens}"
                    )
                ...
                if enforce_planner_policy and {"optimize_ripple", "recover_rejection"}.issubset(allowed_names):
                    ...
                    if decision.get("tool") != expected.get("tool"):
                        raise RuntimeError(
                            f"Planner-policy drift: got {decision.get('tool')}, expected {expected.get('tool')}"
                        )
                    if expected.get("tool") == "optimize_ripple":
                        got_focus = (decision.get("arguments") or {}).get("focus_scenario")
                        exp_focus = (expected.get("arguments") or {}).get("focus_scenario")
                        if got_focus != exp_focus:
                            raise RuntimeError(
                                f"Planner focus drift: got {got_focus}, expected {exp_focus}"
                            )
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="V4.2.4 DeepSeek Flash/Pro FBAW engineering agent with native DSH FBAW-tool capable core with legacy qualification mode."
    )
    ...
    p.add_argument("--deepseek-planner-json-retries", type=int, default=2,
                   help="Retry the real DeepSeek planner when function.arguments is malformed/incomplete JSON; no deterministic fallback is used.")
    p.add_argument("--deepseek-planner-max-tokens", type=int, default=2048,
                   help="Output-token budget for each DeepSeek planner call; larger budgets reduce truncated tool arguments in thinking mode.")
    ...
    p.add_argument("--dsh-diagnostic", action="store_true",
                   help="Run only the DSH launcher + headless model/tool/result/final qualification and exit")
    p.add_argument("--dsh-qualification-test", action="store_true",
                   help="Run DSH qualification first, then the strict multi-step engineering stress test; implies --stress-test --realistic-failure-test and forbids fallback for PASS")
    p.add_argument("--dsh-command", default=os.getenv("DSH_COMMAND", "dsh"),
                   help="Command prefix used to launch DeepSeek Harness, e.g. dsh or D:\\AI_Research\\npm-global\\pnpm.cmd exec dsh")
    p.add_argument("--dsh-workdir", default=os.getenv("DSH_WORKDIR"),
                   help="Working directory for DSH/pnpm. For pnpm exec dsh this must be the workspace containing the DSH package, e.g. D:\\AI_Research\\dsh-run")
    p.add_argument("--dsh-profile", default=os.getenv("DSH_PROFILE", "headless"),
                   help="DSH profile used for the headless qualification probe")
    p.add_argument("--dsh-timeout", type=float, default=180.0,
                   help="Timeout in seconds for the DSH headless qualification probe")
    p.add_argument("--dsh-trace-root", default=os.getenv("DSH_TRACE_ROOT"),
                   help="Optional persisted DSH session/trace directory. If supplied, V4.2.4 scans conservatively for tool/call + tool/result evidence; black-box PASS does not require it.")
    return p.parse_args()
...
def main() -> int:
    ...
    if args.dsh_qualification_test:
        args.stress_test = True
        args.realistic_failure_test = True

    dsh_diag = None
    if args.dsh_diagnostic or args.dsh_qualification_test:
        dsh_diag = run_dsh_harness_diagnostic(
            dsh_command=args.dsh_command,
            dsh_workdir=args.dsh_workdir,
            profile=args.dsh_profile,
            timeout_s=args.dsh_timeout,
            trace_root=args.dsh_trace_root,
        )
        if args.dsh_diagnostic:
            return 0 if dsh_diag.get("dsh_harness_tool_loop_pass") else 3
        if not dsh_diag.get("dsh_harness_tool_loop_pass"):
            print("DSH qualification failed; FBAW engineering run was NOT started.")
            return 3
    ...
    ripple_final, ripple_v, ripple_history, ripple_df, ripple_ok = run_engineering_agent_v4_2(
        ...
        strict_tools=(not args.no_deepseek_strict_tools),
        planner_json_retries=args.deepseek_planner_json_retries,
        planner_max_tokens=args.deepseek_planner_max_tokens,
    ...
    dsh_qualification = {
        "requested": bool(args.dsh_qualification_test),
        "dsh_diagnostic": dsh_diag,
        "dsh_harness_tool_loop_pass": bool((dsh_diag or {}).get("dsh_harness_tool_loop_pass")) if args.dsh_qualification_test else None,
        "black_box_harness_pass": bool((dsh_diag or {}).get("black_box_harness_pass")) if args.dsh_qualification_test else None,
        "trace_verified_pass": bool((dsh_diag or {}).get("trace_verified_pass")) if args.dsh_qualification_test else None,
        "dsh_qualification_level": (dsh_diag or {}).get("qualification_level") if args.dsh_qualification_test else None,
        "engineering_multistep_pass": bool(stress_eval.get("multi_step_pass")),
        "deepseek_tool_calls": int(stress_eval.get("deepseek_tool_calls", 0)),
        "deterministic_fallback_steps": int(stress_eval.get("deterministic_fallback_steps", 0)),
        "no_fallback_pass": bool(stress_eval.get("deterministic_fallback_steps", 0) == 0),
        "overall_pass": bool(
            args.dsh_qualification_test
            and (dsh_diag or {}).get("dsh_harness_tool_loop_pass")
            and stress_eval.get("multi_step_pass")
            and stress_eval.get("deepseek_tool_calls", 0) > 0
            and stress_eval.get("deterministic_fallback_steps", 0) == 0
        ),
        "interpretation": (
            "Overall PASS requires the DSH black-box conditional tool challenge plus the Python-governed DeepSeek multi-step engineering loop. "
            "TRACE_VERIFIED is a stronger optional evidence level. Native DSH registration of FBAW tools is still not claimed."
        ),
    }
    ...
    report = {
        ...
        "agent_version": "V4.2.4_DeepSeek_FlashPro_DSH_Qualification",
        ...
        "dsh_qualification": dsh_qualification,
    ...
    outdir.mkdir(parents=True, exist_ok=True)
    (outdir/"59_3Rx4_ENGINEERING_AGENT_V4_2_3a_DSH_RESULT.json").write_text(
        json.dumps(jsonable(report), indent=2),
        encoding="utf-8"
    )

    print("\n" + "="*78)
    print("3Rx4 AUTONOMOUS ENGINEERING AGENT V4.2.4 DSH QUALIFICATION RESULT")
    print("="*78)
    ...
    if args.dsh_qualification_test:
        print("DSH qualification:")
        print(json.dumps(dsh_qualification, indent=2))
    ...
    print(" ", outdir/"59_3Rx4_ENGINEERING_AGENT_V4_2_3a_DSH_RESULT.json")
    return 0
