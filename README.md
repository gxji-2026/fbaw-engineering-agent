...
The remainder of this section uses the **DSH-native FBAW Engineering Agent** as the primary runtime-verified example.

In this implementation, an LLM planner runs through **DeepSeek Harness (DSH)**, chooses among native FBAW engineering tools, receives Python-verified tool results, re-plans after constraint conflicts or rejected actions, and stops only when the deterministic Python RF core authorizes completion.
