#!/usr/bin/env python3
"""
fbaw_engineering_agent_3Rx4_V4_2_2_deepseek_thinking_tools.py
...
-> DeepSeek advisory -> local refinement proposal
...
V4.3 persistent-bridge/cache upgrade:
    Persistent Python engineering bridge owns verified RF state during the run.
    Design verification uses deterministic memory + disk cache with model/frequency signatures.
    The expensive verified >=50-dB checkpoint is persisted and restored on a valid warm run.
    Cache invalidates on engineering-core/model/frequency changes.
    Runtime statistics report RF simulations, cache hits/misses, and checkpoint status.

V4.2.3 DeepSeek integration/diagnostic foundation retained:
    DeepSeek V4-Flash is the default planner.
    DeepSeek V4-Pro is latched after repeated rollback on a still-failing focus
    or after a Python-simulated realistic constraint conflict.
    Tool names/arguments are schema-validated by Python; strict DeepSeek tool mode
    is enabled by default through the Beta endpoint.
    DeepSeek API preflight/diagnostics run before expensive RF optimization.
    API errors are printed explicitly; silent fallback is disabled by default.
    Output paths are resolved absolutely and recreated before final writes.
...
Optional:
    No extra cloud SDK is required; DeepSeek API calls use Python standard-library HTTPS.

Examples
--------
1) Full Python run using the supplied MBVD seeds, no cloud LLM:
    python fbaw_engineering_agent_3Rx4_V4_2_2_deepseek_thinking_tools.py --no-llm

2) Use COMSOL exports for all three resonators:
    python fbaw_engineering_agent_3Rx4_V4_2_2_deepseek_thinking_tools.py ^
        --comsol-low 6p22.txt ^
        --comsol-high 7p40.txt ^
        --comsol-sfr 7p49.txt

3) Enable DeepSeek V4.2.3 adaptive planner:
...
# =============================================================================
# 5. DEEPSEEK ADVISORY + OPENAI-COMPATIBLE API HELPER
# =============================================================================

DEEPSEEK_BASE_URL = os.getenv("DEEPSEEK_BASE_URL", "https://api.deepseek.com").rstrip("/")
DEEPSEEK_BETA_BASE_URL = os.getenv("DEEPSEEK_BETA_BASE_URL", "https://api.deepseek.com/beta").rstrip("/")
...
def _deepseek_chat_completion(
    *,
    ...
    """Minimal OpenAI-compatible DeepSeek Chat Completions client using stdlib HTTPS."""
    ...
    base = DEEPSEEK_BETA_BASE_URL if strict_tools else DEEPSEEK_BASE_URL
    payload = {
        "model": model,
        "messages": messages,
    ...
    if tools is not None:
        payload["tools"] = tools
    # DeepSeek thinking mode currently rejects forced/named tool_choice.
    # Keep tools available, but let the model select via the default/auto policy;
    # Python still requires and validates exactly one tool call after the response.
    if tool_choice is not None and not thinking:
...
def _masked_key_status() -> str:
    key = os.getenv("DEEPSEEK_API_KEY") or ""
    if not key:
        return "MISSING"
    if len(key) <= 8:
        return f"SET(len={len(key)})"
    return f"SET({key[:3]}...{key[-4:]}, len={len(key)})"


def _deepseek_diagnostic_tool() -> list:
    return [{
        "type": "function",
        "function": {
            "name": "diagnostic_ping",
            "description": "Return one diagnostic tool call to prove structured tool calling is working.",
            "parameters": {
                "type": "object",
                "properties": {
...
def run_deepseek_api_diagnostic(
    *,
    flash_model: str,
    pro_model: str,
    reasoning_effort: str = "high",
    ...
    """Cheap fail-fast connectivity + structured-tool diagnostic before RF work."""
    print("\n" + "="*78)
    print("DEEPSEEK API DIAGNOSTIC / PREFLIGHT")
    print("="*78)
    print("API key:", _masked_key_status())
    print("Base URL:", DEEPSEEK_BETA_BASE_URL if strict_tools else DEEPSEEK_BASE_URL)
    print("Strict tools:", bool(strict_tools))
    print("Reasoning effort:", reasoning_effort)
    print("Thinking/tool-choice policy: AUTO (no forced tool_choice while thinking is enabled)")

    models = [flash_model] + ([pro_model] if test_pro and pro_model != flash_model else [])
    ok = True
    for model in models:
        print(f"\n[DeepSeek preflight] model={model}")
        tools = _deepseek_diagnostic_tool()
        if strict_tools:
            tools = _strict_deepseek_tools(tools)
        try:
            response = _deepseek_chat_completion(
                model=model,
                messages=[
                    {"role": "system", "content": "Call exactly one supplied diagnostic tool. Do not answer in plain text."},
                    {"role": "user", "content": "Call diagnostic_ping with status OK."},
                ],
                tools=tools,
                tool_choice=None,
                reasoning_effort=reasoning_effort,
            ...
            decision, err = _extract_deepseek_tool_call(response)
            if err:
                raise RuntimeError(err)
            valid, why = _validate_tool_call(decision, _deepseek_diagnostic_tool())
            if not valid:
                raise RuntimeError(f"Diagnostic tool call failed Python validation: {why}")
            usage = response.get("usage") or {}
            print("  HTTP/API: OK")
            print("  tool_call:", decision.get("tool"), json.dumps(decision.get("arguments") or {}))
            print("  usage:", json.dumps(usage))
        except Exception as exc:
            ok = False
            print("  HTTP/API: FAILED")
            print("  ERROR:", str(exc))
    print("\nDeepSeek preflight:", "PASS" if ok else "FAIL")
    return ok

def deepseek_advisory(
    selected: dict,
    pareto: List[dict],
    model: str,
    reasoning_effort: str = "high",
) -> dict:
    """DeepSeek sees only Python-verified results and proposes a local experiment."""
    ...
    try:
        ...
        advice = _extract_json_object(msg.get("content") or "")
        advice["status"] = "ok"
        advice["model"] = model
        return advice
    except Exception as exc:
        return {
            ...
            "direction": {"L4_nH": 0, "L6_nH": 0, "C1_pF": 0},
            "window_percent": 3.0,
            "summary": "DeepSeek call failed; deterministic Python fallback used.",
        }
...
# =============================================================================
# Parallel batch verification (opt-in, bit-identical pure parallelism)
# =============================================================================
# The inner sweep loops verify many independent candidate designs and then apply
# an in-order greedy selection. Only the independent verifies are parallelized;
# the selection order and thresholds are unchanged, so results are bit-identical
# to sequential execution (numpy is deterministic per process).
# Set FBAW_WORKERS to the number of worker processes (0 or 1 = sequential,
# the safe default; e.g. `set FBAW_WORKERS=8`).
_FBAW_WORKERS = int(os.getenv("FBAW_WORKERS", "0") or "0")
_PARALLEL_MIN_BATCH = 16
...
def _worker_init(low, high, f_ghz):
    global _W_LOW, _W_HIGH, _W_F
    _W_LOW = low
    _W_HIGH = high
    _W_F = np.asarray(f_ghz, dtype=np.float64)


def _worker_verify_one(args):
    design, extended = args
    if extended:
        return _verify_cellwise_extended_raw(design, _W_LOW, _W_HIGH, _W_F)
    return verify_cellwise(design, _W_LOW, _W_HIGH, _W_F)


def _get_pool(workers, low, high, f_ghz):
    """Persistent worker pool: spawn once, reuse across all sweep batches."""
    global _POOL, _POOL_CTX
    key = (id(low), id(high), id(f_ghz))
    if _POOL is None or _POOL_CTX != key:
        if _POOL is not None:
            _POOL.shutdown(wait=False, cancel_futures=True)
        from concurrent.futures import ProcessPoolExecutor
        _POOL = ProcessPoolExecutor(max_workers=workers, initializer=_worker_init,
                                    initargs=(low, high, f_ghz))
        _POOL_CTX = key
    return _POOL


def _verify_batch(designs, low, high, f_ghz, extended=False):
    """Verify a list of designs in order; parallel when enabled and worthwhile."""
    if not designs:
        return []
    workers = _FBAW_WORKERS
    # P3: extended verification is owned by the bridge even when workers do
    # the raw numerical work.  The bridge performs lookup, accounting and
    # cache commit in the parent process.
    if (extended and _V43_BRIDGE is not None and workers > 1
            and len(designs) >= _PARALLEL_MIN_BATCH):
        return _V43_BRIDGE.batch_verify(designs, low, high, f_ghz, workers)
    if workers <= 1 or len(designs) < _PARALLEL_MIN_BATCH:
        # Sequential path keeps the original bridge-cached entry point for the
        # extended scenario, so default-mode statistics/behaviour are unchanged.
        if extended:
            return [verify_cellwise_extended(d, low, high, f_ghz) for d in designs]
        return [verify_cellwise(d, low, high, f_ghz) for d in designs]
    workers = min(workers, os.cpu_count() or 1, len(designs))
    pool = _get_pool(workers, low, high, f_ghz)
    return list(pool.map(_worker_verify_one, [(d, extended) for d in designs],
                         chunksize=max(1, len(designs) // (workers * 4))))
    ...
    for sweep_pass in range(3):
        ...
        for pidx, name in enumerate(names):
            ...
            for val in vals:
                x = x0.copy()
                x[pidx] = val
                cands.append(replace_cell(current, cell_index, _cell_from_vector(x)))
            vvs = _verify_batch(cands, low, high, f_ghz)

            for val, cand, vv in zip(vals, cands, vvs):
                mn = vv["nominal"]
                rows.append({
                    "cell": cell_index + 1,
...
def optimize_terminal_2d(
    ...
    cands = [
        replace_terminals(base, L4_nH=l4, L6_nH=l6)
        for l4 in l4vals for l6 in l6vals
    ]
    vvs = _verify_batch(cands, low, high, f_ghz)
    for cand, vv in zip(cands, vvs):
        if not terminal_guard(vv):
...
def l6_directed_extension(
    ...
    cands = [replace_terminals(current, L6_nH=float(l6)) for l6 in l6_values]
    vvs = _verify_batch(cands, low, high, f_ghz)
    for cand, vv in zip(cands, vvs):
    ...
    cands = [replace_terminals(current, L4_nH=float(l4)) for l4 in l4_values]
    vvs = _verify_batch(cands, low, high, f_ghz)
    for cand, vv in zip(cands, vvs):
        if not l6ext_guard(vv):
            continue
    ...
    cands = [
        replace_terminals(current, L4_nH=float(l4), L6_nH=float(l6))
        for l4 in l4_values for l6 in l6_values
    ]
    vvs = _verify_batch(cands, low, high, f_ghz)
    for cand, vv in zip(cands, vvs):
        if not l6ext_guard(vv):
...
V43_CACHE_SCHEMA = "V4.3-RF-CACHE-1"
V43_SOURCE_SHA256 = hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
_V43_BRIDGE = None


def _canonical_hash(obj) -> str:
    raw = json.dumps(jsonable(obj), sort_keys=True, separators=(",", ":"), allow_nan=True).encode("utf-8")
    return hashlib.sha256(raw).hexdigest()


def _mbvd_signature(x: MBVD) -> dict:
    return asdict(x)


def _cellwise_from_dict(d: dict) -> CellwiseDesign:
    cells_d = d.get("cells") or {}
    cells = []
    for i in range(1, 5):
        c = cells_d[f"cell{i}"]
...
class PersistentEngineeringBridge:
    """Persistent deterministic Python execution/state/cache layer for V4.3."""

    def __init__(self, cache_dir: Path, enabled: bool = True):
        self.cache_dir = Path(cache_dir).expanduser().resolve()
        self.enabled = bool(enabled)
        self.cache_dir.mkdir(parents=True, exist_ok=True)
        self.verify_cache_path = self.cache_dir / "verification_cache_v43_p3.pkl"
        self.checkpoint_cache_path = self.cache_dir / "checkpoint_v43_p3.pkl"
        self.verify_cache = {}
        self.stats = {
            "verify_requests": 0,
            "verify_cache_hits": 0,
            "verify_cache_misses": 0,
            "rf_scenario_simulations": 0,
            "checkpoint_status": "disabled" if not self.enabled else "not_checked",
            "checkpoint_hits": 0,
            "checkpoint_rebuilds": 0,
            "parallel_batches": 0,
            "parallel_candidates": 0,
            "parallel_unique_misses": 0,
            "parallel_intrabatch_hits": 0,
        }
        self.current_design = None
        self.current_verified = None
        self.started_at = time.perf_counter()
        if self.enabled:
            self._load_verify_cache()

    def _load_verify_cache(self):
        if not self.verify_cache_path.is_file():
            return
        try:
            payload = pickle.loads(self.verify_cache_path.read_bytes())
            if payload.get("schema") == V43_CACHE_SCHEMA:
                self.verify_cache = payload.get("entries") or {}
        except Exception:
            self.verify_cache = {}
    ...
    def flush(self):
        if not self.enabled:
            return
        payload = {"schema": V43_CACHE_SCHEMA, "entries": self.verify_cache}
        tmp = self.verify_cache_path.with_suffix(".tmp")
        tmp.write_bytes(pickle.dumps(payload, protocol=pickle.HIGHEST_PROTOCOL))
        tmp.replace(self.verify_cache_path)

    def clear_verification_cache(self):
        self.verify_cache = {}
        if self.enabled and self.verify_cache_path.is_file():
            self.verify_cache_path.unlink()

    def verification_key(self, design: CellwiseDesign, low: MBVD, high: MBVD, f_ghz: np.ndarray) -> str:
        f = np.asarray(f_ghz, dtype=np.float64)
        payload = {
            "schema": V43_CACHE_SCHEMA,
            "core_source_sha256": V43_SOURCE_SHA256,
            "design": design.to_dict(),
            "low": _mbvd_signature(low),
            "high": _mbvd_signature(high),
            "frequency": {
                "n": int(f.size),
                "first": float(f[0]),
                "last": float(f[-1]),
            ...
            "topology": "3Rx4_cellwise_ADS_exact_external_inductor_Cp_full_each_segment",
        }
        return _canonical_hash(payload)

    def verify(self, design: CellwiseDesign, low: MBVD, high: MBVD, f_ghz: np.ndarray) -> dict:
        self.stats["verify_requests"] += 1
        key = self.verification_key(design, low, high, f_ghz)
        if self.enabled and key in self.verify_cache:
            self.stats["verify_cache_hits"] += 1
            result = self.verify_cache[key]
        else:
            self.stats["verify_cache_misses"] += 1
            result = _verify_cellwise_extended_raw(design, low, high, f_ghz)
            self.stats["rf_scenario_simulations"] += 3
            if self.enabled:
                self.verify_cache[key] = result
        self.current_design = design
        self.current_verified = result
        return result

    def batch_verify(self, designs, low, high, f_ghz, workers: int):
        """P3 parent-owned cached batch verification, preserving input order."""
        designs = list(designs)
        if not designs:
            return []
        self.stats["parallel_batches"] += 1
        self.stats["parallel_candidates"] += len(designs)
        ...
        for i, design in enumerate(designs):
            ...
            if self.enabled and key in self.verify_cache:
                self.stats["verify_cache_hits"] += 1
                results[i] = self.verify_cache[key]
            elif key in pending:
                # A duplicate request in one batch shares the first raw result.
                self.stats["verify_cache_hits"] += 1
                self.stats["parallel_intrabatch_hits"] += 1
                pending[key][1].append(i)
            else:
                self.stats["verify_cache_misses"] += 1
                pending[key] = [design, [i]]

        unique = list(pending.items())
        self.stats["parallel_unique_misses"] += len(unique)
        if unique:
            nworkers = min(max(1, int(workers)), os.cpu_count() or 1, len(unique))
            pool = _get_pool(nworkers, low, high, f_ghz)
            raw = list(pool.map(
                _worker_verify_one,
                [(item[1][0], True) for item in unique],
                chunksize=max(1, len(unique) // (nworkers * 4)),
            ))
            for ((key, (_, indices)), value) in zip(unique, raw):
                self.stats["rf_scenario_simulations"] += 3
                if self.enabled:
                    self.verify_cache[key] = value
                for i in indices:
                    results[i] = value

        self.current_design = designs[-1]
        self.current_verified = results[-1]
        return results

    def checkpoint_fingerprint(self, low: MBVD, high: MBVD, f_ghz: np.ndarray, freq_points: int) -> str:
        # Schema is intentionally explicit: any algorithm/model change should bump it.
        payload = {
            "schema": V43_CACHE_SCHEMA,
            "core_source_sha256": V43_SOURCE_SHA256,
            "checkpoint_algorithm": "uniform->coarse->fine->terminal->L6->Q60->balanced->recovery->last->ultrafine->L3ultra",
            "low": _mbvd_signature(low),
            "high": _mbvd_signature(high),
            "freq_points": int(freq_points),
            "frequency_sha256": hashlib.sha256(np.asarray(f_ghz, dtype=np.float64).tobytes()).hexdigest(),
            "passband": PASSBAND_GHZ,
            "stopbands": [LOW_STOP_GHZ, HIGH_STOP_GHZ],
            "cellwise_start": CELLWISE_START,
        }
        return _canonical_hash(payload)

    def load_checkpoint(self, fingerprint: str):
        if not self.enabled or not self.checkpoint_cache_path.is_file():
            self.stats["checkpoint_status"] = "miss"
            return None
        try:
            payload = pickle.loads(self.checkpoint_cache_path.read_bytes())
            if payload.get("schema") != V43_CACHE_SCHEMA or payload.get("fingerprint") != fingerprint:
                self.stats["checkpoint_status"] = "invalidated"
                return None
            self.stats["checkpoint_status"] = "hit"
            self.stats["checkpoint_hits"] += 1
            return payload.get("bundle")
        except Exception:
            self.stats["checkpoint_status"] = "corrupt_rebuild"
            return None

    def save_checkpoint(self, fingerprint: str, bundle: dict):
        if not self.enabled:
            return
        payload = {"schema": V43_CACHE_SCHEMA, "fingerprint": fingerprint, "bundle": bundle}
        tmp = self.checkpoint_cache_path.with_suffix(".tmp")
        tmp.write_bytes(pickle.dumps(payload, protocol=pickle.HIGHEST_PROTOCOL))
        tmp.replace(self.checkpoint_cache_path)
        self.stats["checkpoint_status"] = "rebuilt"
        self.stats["checkpoint_rebuilds"] += 1

    def state_snapshot(self) -> dict:
        elapsed = time.perf_counter() - self.started_at
        req = self.stats["verify_requests"]
        hits = self.stats["verify_cache_hits"]
        return {
            **self.stats,
            "verification_cache_entries": len(self.verify_cache),
            "verify_hit_rate": (hits / req) if req else 0.0,
            "elapsed_s": float(elapsed),
            "current_design_present": self.current_design is not None,
        }


def _verify_cellwise_extended_raw(
    design: CellwiseDesign,
    low: MBVD,
    high: MBVD,
    f_ghz: np.ndarray,
) -> dict:
    """Uncached numerical authority used internally by the V4.3 bridge."""
    out = {}
    for name, q, cp in (
        ("nominal", None, 0.0),
        ("Q60_Cp60", 60.0, 60.0),
...
def verify_cellwise_extended(
    ...
    """V4.3 verification entry point; all callers transparently use the persistent bridge."""
    global _V43_BRIDGE
    if _V43_BRIDGE is None:
        return _verify_cellwise_extended_raw(design, low, high, f_ghz)
    return _V43_BRIDGE.verify(design, low, high, f_ghz)
...
def collect_and_select_balanced_checkpoint(
    ...
    designs = [d for _, d in candidates]
    vvs = _verify_batch(designs, low, high, f_ghz, extended=True)

    for (label, d), v in zip(candidates, vvs):
    ...
    for _ in range(2):
        ...
        for name in RECOVERY_NAMES:
            ...
            for val in vals:
                ...
                x[pidx] = float(val)
                cands.append(replace_cell(current, cell_index, _cell_from_vector(x)))
            vvs = _verify_batch(cands, low, high, f_ghz, extended=True)
...
def _last_scan_parameter(
    ...
    for val in vals:
        x = x0.copy()
        x[pidx] = float(val)
        cands.append(replace_cell(base, cell_index, _cell_from_vector(x)))
    vvs = _verify_batch(cands, low, high, f_ghz, extended=True)
...
def _joint_l4l6_micro_retune(
    ...
    def run_grid(center: CellwiseDesign, frac: float):
        ...
        cands = [
            replace_terminals(base, L4_nH=float(l4), L6_nH=float(l6))
            for l4 in l4_vals for l6 in l6_vals
        ]
        vvs = _verify_batch(cands, low, high, f_ghz, extended=True)
        for cand, vv in zip(cands, vvs):
            if not last_micro_guard(vv):
...
def ultrafine_global_l4l6_closure(
    ...
    for level, frac in enumerate(ULTRA_L4L6_WINDOWS, start=1):
        ...
        feasible = []
        grid = [
            (float(l4), float(l6), replace_terminals(start, L4_nH=float(l4), L6_nH=float(l6)))
            for l4 in l4_vals for l6 in l6_vals
        ]
        vvs = _verify_batch([c for _, _, c in grid], low, high, f_ghz, extended=True)
        for (l4, l6, cand), vv in zip(grid, vvs):
...
def _l4l6_compensate_after_l3(
    ...
    rows = []
    grid = [
        (float(l4), float(l6), replace_terminals(base, L4_nH=float(l4), L6_nH=float(l6)))
        for l4 in l4_vals for l6 in l6_vals
    ]
    vvs = _verify_batch([c for _, _, c in grid], low, high, f_ghz, extended=True)
    for (l4, l6, cand), vv in zip(grid, vvs):
...
def ultramicro_l3_with_l4l6_compensation(
    ...
    for cycle, frac in enumerate(L3_ULTRA_WINDOWS, start=1):
        ...
        for ci in L3_ULTRA_CELL_ORDER:
            ...
            feasible = []
            cands = [_replace_cell_l3(current, ci, float(val)) for val in vals]
            vvs = _verify_batch(cands, low, high, f_ghz, extended=True)
            for val, cand, vv in zip(vals, cands, vvs):
                surface_rows.append({
...
def _r45_terminal_search(
    ...
    rows = []
    grid = [
        (float(l4), float(l6), replace_terminals(current, L4_nH=float(l4), L6_nH=float(l6)))
        for l4 in l4vals for l6 in l6vals
    ]
    vvs = _verify_batch([c for _, _, c in grid], low, high, f_ghz, extended=True)
    for (l4, l6, cand), vv in zip(grid, vvs):
        ...
        rows.append({
            ...
            "window_percent": 100*frac, "L4_nH": l4, "L6_nH": l6,
    ...
    for cycle, frac in enumerate(R45_WINDOWS, start=1):
        ...
        for ci in R45_CELL_ORDER:
            for par in R45_PARAMETER_ORDER:
                ...
                best = None
                cands = [_replace_cell_parameter(current, ci, par, float(val)) for val in vals]
                vvs = _verify_batch(cands, low, high, f_ghz, extended=True)
                for val, cand, vv in zip(vals, cands, vvs):
                    feasible = _r45_guard(vv, branch_start_il)
                    surface.append({
...
# =============================================================================
# 7B. AUTONOMOUS ENGINEERING AGENT V4.2 — DEEPSEEK FLASH/PRO ADAPTIVE PLANNER
# =============================================================================
...
def deterministic_planner(v: dict, goal: AgentGoal) -> dict:
    """Offline / API-failure fallback. Returns the same action vocabulary as DeepSeek tools."""
    ev = evaluate_agent_goal(v, goal)
    if ev["satisfied"]:
        return {
...
def _tool_subset(*names: str) -> list:
    """Return only the named DeepSeek tools, preserving their schemas."""
    wanted = set(names)
    return [
        t for t in ENGINEERING_TOOLS
...
def _stress_allowed_tools(
    ...
    """Enforced V4.2 evaluation state machine."""
    tools_seen = [
...
def _deepseek_tool_result_message(tool_call_id: str, payload: dict) -> dict:
    """OpenAI-compatible DeepSeek tool-result message."""
    return {
        "role": "tool",
        "tool_call_id": tool_call_id,
        "content": json.dumps(jsonable(payload)),
    }


def _deepseek_assistant_message(response: dict) -> dict:
    """Preserve tool_calls and reasoning_content for DeepSeek thinking-mode continuation."""
    msg = dict(((response.get("choices") or [{}])[0].get("message") or {}))
    allowed = {"role", "content", "tool_calls", "reasoning_content", "name"}
    out = {k: v for k, v in msg.items() if k in allowed and v is not None}
    out.setdefault("role", "assistant")
    # Keep an explicit content field for tool-loop compatibility.
    out.setdefault("content", "")
    return out


def _deepseek_continue_user_message(current_v: dict, goal: AgentGoal) -> dict:
    """Force the next sub-turn to continue the engineering tool loop from CURRENT verified state."""
    return {
        "role": "user",
        "content": (
            "Continue the V4.2.3 engineering-agent evaluation from the returned Python-verified CURRENT state. "
            "Do not give a prose-only answer and do not declare completion yourself. "
            "Call exactly ONE of the currently supplied tools. "
            "Python alone controls ACCEPT, ROLLBACK, and STOP. Current verified evaluation: "
            + json.dumps(jsonable(evaluate_agent_goal(current_v, goal)))
        ),
    }


def _deepseek_retry_tool_call_message(parse_error: str) -> dict:
    """One corrective user turn when thinking+auto returns no tool call."""
    return {
        "role": "user",
        "content": (
            "Your previous response did not provide exactly one valid tool call (" + str(parse_error) + "). "
            "Do not answer in prose. Call exactly ONE of the supplied tools now, using only its schema. "
            "Do not invent RF metrics, tools, or arguments."
        ),
    }


def _extract_deepseek_tool_call(response: dict) -> tuple[Optional[dict], Optional[str]]:
    msg = ((response.get("choices") or [{}])[0].get("message") or {})
    calls = msg.get("tool_calls") or []
    if len(calls) != 1:
        return None, f"Expected exactly one tool call, got {len(calls)}"
    tc = calls[0]
    ...
    try:
        ...
        return None, f"Invalid tool JSON arguments: {exc}"
    return {
        "tool": name,
        "arguments": args,
        "tool_call_id": tc.get("id", "unknown_tool_call"),
        "source": "deepseek_tool_call",
    }, None


def _strict_deepseek_tools(tools: list) -> list:
    """Return DeepSeek strict-mode schemas (Beta): strict=true + no extra properties."""
    ...
    for t in out:
        ...
        if params.get("type") == "object":
            ...
            # DeepSeek strict mode requires all defined object properties to be required.
            params["required"] = list(props.keys())
    return out


def _validate_tool_call(decision: dict, allowed_tools: list) -> tuple[bool, str]:
    """Python-side allowlist/schema guard; the LLM never gets execution authority."""
    if not isinstance(decision, dict):
        return False, "planner decision is not an object"
    name = decision.get("tool")
    allowed = {t.get("function", {}).get("name"): t.get("function", {}) for t in allowed_tools}
    if name not in allowed:
        return False, f"tool {name!r} is not in the allowed tool set"
    args = decision.get("arguments") or {}
    if not isinstance(args, dict):
        return False, "tool arguments must be an object"
    schema = (allowed[name].get("parameters") or {})
    ...
    for key, spec in props.items():
        ...
        if "enum" in spec and value not in spec["enum"]:
            return False, f"argument {key}={value!r} is outside enum {spec['enum']}"
    return True, "ok"


def _expected_policy_decision(current_v: dict, goal: AgentGoal) -> dict:
    """Authoritative V4.2 policy used to reject planner-policy drift."""
    return deterministic_planner(current_v, goal)


def _focus_from_trace_record(rec: dict) -> Optional[str]:
    planner = rec.get("planner", {}) or {}
    tool = planner.get("tool")
    args = planner.get("arguments") or {}
    if tool == "optimize_ripple":
        return args.get("focus_scenario")
    if tool == "recover_rejection":
        return "rejection"
    return None


def deepseek_escalation_route(
    trace: list,
    current_v: dict,
    goal: AgentGoal,
    rollback_threshold: int = 2,
) -> dict:
    """Route ordinary planning to Flash and repeated/conflicted failures to Pro."""
    ev = evaluate_agent_goal(current_v, goal)
    if ev["satisfied"]:
        return {"route": "FLASH", "reason": "Current Python-verified state is satisfied."}

    failed_focuses = []
    if not ev["checks"]["rejection"]:
        failed_focuses.append("rejection")
    for check, focus in (
        ("nominal_ripple", "nominal"),
        ("Q80_Cp40_ripple", "Q80_Cp40"),
        ("Q60_Cp60_ripple", "Q60_Cp60"),
    ):
        if not ev["checks"][check]:
            failed_focuses.append(focus)

    recent = trace[-8:]
    for focus in failed_focuses:
        n = sum(
            1 for r in recent
            if bool((r.get("tool_result", {}) or {}).get("rollback"))
            and _focus_from_trace_record(r) == focus
        )
        if n >= max(1, int(rollback_threshold)):
            return {
                "route": "PRO",
                "reason": f"{n} recent rollbacks while {focus} remains unsatisfied.",
                "focus": focus,
                "rollback_count": n,
            }

    if any(
        bool((r.get("tool_result", {}) or {}).get("realistic_failure_observed"))
        for r in recent
    ):
        return {
            "route": "PRO",
            "reason": "Recent Python-simulated constraint conflict requires escalated planning.",
        }
    return {"route": "FLASH", "reason": "No escalation trigger is active."}


def make_deepseek_agent_client():
    """Compatibility hook: API transport is provided by _deepseek_chat_completion."""
    if not os.getenv("DEEPSEEK_API_KEY"):
...
def execute_engineering_tool(
    ...
    Safety/reliability invariant:
      - DeepSeek never supplies component values or RF metrics.
...
def run_engineering_agent_v4_3(
    start,
    low: MBVD,
    high: MBVD,
    f_ghz: np.ndarray,
    ...
    escalation_rollbacks: int = 2,
    strict_tools: bool = True,
    enforce_planner_policy: bool = True,
    ...
    """
    DeepSeek V4.3 adaptive loop:
      Flash planner -> optional Pro escalation -> strict tool call -> Python RF tool
      -> Python evaluator -> accept/rollback -> exact tool result -> re-plan or STOP.

    Python remains the numerical, acceptance, rollback, policy-gating, and STOP authority.
    """
    current = start
    ...
    deepseek_error = None
    if use_deepseek:
        ds, deepseek_error = make_deepseek_agent_client()

    system = (
        "You are the planning layer of an autonomous, physics-constrained FBAW engineering agent. "
        "You MUST use exactly one of the supplied tools. Never invent S-parameters, component values, "
        "performance, tools, or tool arguments. Python is the numerical and acceptance authority. "
        "Authoritative planner priority: if satisfied=true request stop_if_satisfied; else if rejection fails "
        "choose recover_rejection; otherwise choose optimize_ripple for the verified ripple scenario with the "
        "worst margin. After rollback, plan only from the returned CURRENT Python-verified state."
    )
    if stress_test:
        system += (
            ...
            "When all goals are satisfied, explicitly call stop_if_satisfied."
        )
        if failure_recovery_test:
            system += (
                " FAILURE-RECOVERY PROTOCOL: after inspection, call controlled_failure_probe, then re-plan."
            )
        if realistic_failure_test:
            system += (
                " REALISTIC-FAILURE PROTOCOL: after inspection, call realistic_constraint_conflict_probe. "
                "Read the exact rollback evidence, then re-plan using only the preserved verified state."
            )
        user_content = (
            "Run the multi-step agent evaluation. Hard goal: "
            + json.dumps(jsonable(asdict(goal)))
            + ". Inspect the verified state first."
        )
    else:
        user_content = (
            "Optimize this FBAW filter to the hard goal. Current state is Python-verified:\n"
    ...
    for step in range(1, max_agent_steps + 1):
        ...
        route = deepseek_escalation_route(
            trace, current_v, goal, rollback_threshold=escalation_rollbacks
        )
        if route.get("route") == "PRO":
            pro_latched = True
        planner_model = pro_model if pro_latched else flash_model
        ...
        allowed_tools = (
            ...
            if stress_test else ENGINEERING_TOOLS
        )
        ...
        if ds is not None:
            try:
                response = _deepseek_chat_completion(
                    model=planner_model,
                    messages=messages,
                    tools=_strict_deepseek_tools(allowed_tools) if strict_tools else allowed_tools,
                    tool_choice=None,
                    reasoning_effort=reasoning_effort,
                    thinking=True,
                    strict_tools=strict_tools,
                )
                decision, parse_error = _extract_deepseek_tool_call(response)

                # DeepSeek thinking + auto tool selection may legally return prose with zero
                # tool calls after a tool result. Preserve that assistant turn (including
                # reasoning_content), add one explicit continuation request, and retry once.
                if parse_error:
                    first_msg = _deepseek_assistant_message(response)
                    first_calls = first_msg.get("tool_calls") or []
                    if len(first_calls) == 0:
                        print("DeepSeek continuous tool-loop retry:", parse_error)
                        # The first attempt (thinking mode + auto tool selection) returned
                        # prose with zero tool calls. Retry WITHOUT thinking and FORCE a
                        # tool call so the API cannot legally return a zero-tool-call reply.
                        first_msg.pop("reasoning_content", None)
                        messages.append(first_msg)
                        messages.append(_deepseek_retry_tool_call_message(parse_error))
                        response = _deepseek_chat_completion(
                            model=planner_model,
                            messages=messages,
                            tools=_strict_deepseek_tools(allowed_tools) if strict_tools else allowed_tools,
                ...
                valid, why = _validate_tool_call(decision, allowed_tools)
                if not valid:
                    raise RuntimeError(f"Rejected DeepSeek tool call: {why}")

                # In the normal optimization/recovery phase, Python enforces V4.2 policy.
                allowed_names = {t.get("function", {}).get("name") for t in allowed_tools}
                if enforce_planner_policy and {"optimize_ripple", "recover_rejection"}.issubset(allowed_names):
                    expected = _expected_policy_decision(current_v, goal)
                    drifted = (
                        decision.get("tool") != expected.get("tool")
                        or (
                            decision.get("tool") == "optimize_ripple"
                            and (decision.get("arguments") or {}).get("focus_scenario")
                                != (expected.get("arguments") or {}).get("focus_scenario")
                        )
                    )
                    if drifted:
                        # Python is the policy authority: correct the drift in-place with the
                        # deterministic policy decision instead of failing the whole run. The
                        # corrected action is Python-verified, so correctness is unchanged.
                        print(
                            "Planner-policy drift corrected by Python: got "
                            f"tool={decision.get('tool')} args={decision.get('arguments')} -> "
                            f"expected tool={expected.get('tool')} args={expected.get('arguments')}"
                        )
                        decision["tool"] = expected.get("tool")
                        decision["arguments"] = dict(expected.get("arguments") or {})
                        decision["policy_corrected"] = True

                decision["planner_model"] = planner_model
                decision["router"] = route
                messages.append(_deepseek_assistant_message(response))
            except Exception as exc:
                deepseek_error = str(exc)
                print("DeepSeek API/planner error:", deepseek_error)
                print("DeepSeek attempted model:", planner_model)
                print("DeepSeek attempted endpoint:", (DEEPSEEK_BETA_BASE_URL if strict_tools else DEEPSEEK_BASE_URL) + "/chat/completions")
                print("DeepSeek strict tools:", bool(strict_tools))
                if not allow_deterministic_fallback:
                    raise RuntimeError(
                        "DeepSeek planner failed and deterministic fallback is disabled. "
                        "Use --allow-deterministic-fallback only when fallback behavior is intentionally desired."
                    ) from exc
                decision = None
        ...
        if decision is None:
            if stress_test:
                ...
                if not inspection_seen:
                    ...
                    decision = {
                        "tool": "realistic_constraint_conflict_probe",
                        "arguments": {"reason": "V4.2 requires one real simulated constraint-conflict probe before recovery."},
                        "source": "deterministic_fallback",
                    }
                elif failure_recovery_test and not failure_seen:
                    decision = {
                        "tool": "controlled_failure_probe",
                        "arguments": {"reason": "V4.2 requires one controlled failure before recovery."},
                        "source": "deterministic_fallback",
                    }
                ...
                decision = deterministic_planner(current_v, goal)
            decision["planner_model"] = "deterministic_fallback"
            decision["router"] = route
            if deepseek_error:
                decision["deepseek_error"] = deepseek_error

        tool_name = decision.get("tool")
        tool_args = decision.get("arguments") or {}
        print("\n" + "="*78)
        print(f"DEEPSEEK ENGINEERING AGENT V4.2.3 STEP {step}")
        print("="*78)
        print("Planner:", decision.get("source"))
        print("Planner model:", decision.get("planner_model"))
        print("Router:", json.dumps(decision.get("router") or {}))
        if stress_test:
        ...
        if not sdf.empty:
            ...
            sdf["planner_model"] = decision.get("planner_model")
            dfs.append(sdf)
        ...
        if ds is not None and decision.get("source") == "deepseek_tool_call":
            messages.append(_deepseek_tool_result_message(
                decision.get("tool_call_id", "unknown_tool_call"),
                tool_result,
            ))
            # Bridge the completed tool sub-turn into the next autonomous planner turn.
            # This prevents an auto-tool model from interpreting the tool result as a cue
            # to finish with prose instead of selecting the next engineering action.
            if not (tool_name == "stop_if_satisfied" and tool_result.get("stop_allowed")):
                messages.append(_deepseek_continue_user_message(current_v, goal))
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="V4.3 DeepSeek Flash/Pro FBAW engineering agent with persistent Python bridge and deterministic cache."
    )
    ...
    p.add_argument(
        ...
        default=os.getenv("DEEPSEEK_FLASH_MODEL", "deepseek-v4-flash"),
    )
    p.add_argument(
        "--deepseek-pro-model",
        default=os.getenv("DEEPSEEK_PRO_MODEL", "deepseek-v4-pro"),
    )
    p.add_argument(
        "--deepseek-reasoning-effort", choices=("low", "high", "max"),
        default=os.getenv("DEEPSEEK_REASONING_EFFORT", "high"),
    )
    p.add_argument("--deepseek-escalation-rollbacks", type=int, default=2,
                   help="Escalate Flash to Pro after this many recent rollbacks on a still-failing focus")
    p.add_argument("--no-deepseek-strict-tools", action="store_true",
                   help="Disable DeepSeek Beta strict tool schemas; Python validation remains active")
    p.add_argument("--allow-planner-policy-deviation", action="store_true",
                   help="Do not reject a schema-valid DeepSeek action that differs from deterministic V4.2 priority")
    ...
    p.add_argument("--cache-dir", default=".fbaw_v43_p3_cache",
                   help="Independent persistent V4.3-P3 engineering cache directory")
    p.add_argument("--no-cache", action="store_true",
                   help="Disable V4.3 verification/checkpoint caches")
    p.add_argument("--rebuild-cache", action="store_true",
                   help="Ignore the saved checkpoint and rebuild it; verification cache remains available")
    p.add_argument("--cold-rebuild", action="store_true",
                   help="Clear P3 verification cache and rebuild the checkpoint")
    p.add_argument("--regression-check", nargs="?", metavar="OPTIONAL_V43_RESULT_JSON",
                   const="embedded", default="embedded",
                   help="Run regression (default: embedded certified V4.3 golden metrics); optionally supply a V4.3 result JSON for full design comparison")
    p.add_argument("--no-regression-check", dest="regression_check", action="store_const", const=None,
                   help="Disable the automatic P3 regression gate")
    p.add_argument("--recover-regression-cold", metavar="COMPLETED_P3_RESULT_JSON",
                   help="Validate an already completed P3 cold result and record it without rerunning RF/DeepSeek; requires --regression-check")
    p.add_argument("--regression-design-atol", type=float, default=1e-12)
    p.add_argument("--regression-metric-atol", type=float, default=2e-6)
    ...
    p.add_argument("--no-deepseek-planner", action="store_true",
                   help="Use deterministic planner fallback; still executes the same Python tools")
    p.add_argument("--deepseek-api-diagnostic", action="store_true",
                   help="Run only the DeepSeek Flash/Pro structured-tool API diagnostic and exit")
    p.add_argument("--skip-deepseek-preflight", action="store_true",
                   help="Skip automatic DeepSeek API preflight before expensive RF optimization")
    p.add_argument("--allow-deterministic-fallback", action="store_true",
                   help="Allow deterministic fallback if DeepSeek API/planner fails; default is fail-closed")
    ...
    p.add_argument("--failure-recovery-test", action="store_true",
                   help="V4.2 reliability test: inspect -> controlled failure -> recovery re-plan -> explicit STOP; implies --stress-test")
    p.add_argument("--realistic-failure-test", action="store_true",
                   help="V4.2 realistic test: inspect -> real simulated constraint-conflict rejection/rollback -> recovery re-plan -> explicit STOP; implies --stress-test")
    return p.parse_args()


_REGRESSION_METRICS = (
    "passband_ripple_dB", "minimum_IL_dB", "worst_passband_IL_dB",
    "worst_S11_dB", "BW_3dB_GHz", "lower_stopband_min_rejection_dB",
    "upper_stopband_min_rejection_dB",
)

# Certified by the completed V4.3 <-> V4.3-P3 regression run.  Full-precision
# quantities printed to six decimals use a 2e-6 default tolerance; stopband
# values were printed to three decimals and therefore use a 1e-3 tolerance.
_EMBEDDED_V43_GOLDEN_METRICS = {
    "nominal": {
        "passband_ripple_dB": 0.539940,
        "minimum_IL_dB": 0.345593,
        "worst_passband_IL_dB": 0.885533,
        "worst_S11_dB": -8.652179,
        "BW_3dB_GHz": 0.838100,
        "lower_stopband_min_rejection_dB": 51.177,
        "upper_stopband_min_rejection_dB": 55.633,
    },
    "Q80_Cp40": {
        "passband_ripple_dB": 0.618137,
        "minimum_IL_dB": 1.288908,
        "worst_passband_IL_dB": 1.907045,
        "worst_S11_dB": -8.675837,
        "BW_3dB_GHz": 0.823600,
        "lower_stopband_min_rejection_dB": 50.583,
        "upper_stopband_min_rejection_dB": 56.093,
    },
    "Q60_Cp60": {
        "passband_ripple_dB": 0.745393,
        "minimum_IL_dB": 1.559092,
        "worst_passband_IL_dB": 2.304484,
        "worst_S11_dB": -8.000172,
        "BW_3dB_GHz": 0.814900,
        "lower_stopband_min_rejection_dB": 50.240,
        "upper_stopband_min_rejection_dB": 56.311,
    },
}
...
def _numeric_leaves(value, prefix=""):
    ...
    if isinstance(value, dict):
        for key, item in value.items():
            out.update(_numeric_leaves(item, f"{prefix}.{key}" if prefix else str(key)))
    elif isinstance(value, (int, float)) and not isinstance(value, bool):
        out[prefix] = float(value)
    return out


def _regression_compare_map(name, expected, actual, atol, failures):
    e = _numeric_leaves(expected)
    a = _numeric_leaves(actual)
    if set(e) != set(a):
        failures.append(f"{name}: numeric field set differs")
        return
    bad = [(key, e[key], a[key]) for key in e if abs(e[key] - a[key]) > atol]
    if bad:
        key, ev, av = bad[0]
        failures.append(f"{name}: {key} expected {ev:.15g}, got {av:.15g}")


def run_regression_check(report, bridge_stats, baseline_path, cache_dir,
                         design_atol, metric_atol):
    """Small post-run regression gate; never starts an additional RF sweep."""
    failures = []
    embedded = baseline_path == "embedded"
    baseline = None if embedded else json.loads(
        Path(baseline_path).expanduser().read_text(encoding="utf-8")
    )
    if baseline is not None:
        _regression_compare_map("final design", baseline["final_parameters"],
                                report["final_parameters"], design_atol, failures)
    for scenario in ("nominal", "Q80_Cp40", "Q60_Cp60"):
        source = _EMBEDDED_V43_GOLDEN_METRICS if embedded else baseline["final_metrics"]
        for key in _REGRESSION_METRICS:
            expected = float(source[scenario][key])
            actual = float(report["final_metrics"][scenario][key])
            atol = max(metric_atol, 1e-3) if "stopband" in key else metric_atol
            if abs(expected - actual) > atol:
                failures.append(
                    f"{scenario} metrics: {key} expected {expected:.15g}, got {actual:.15g}"
                )
                break

    stress = report["stress_test_evaluation"]
    seq = stress.get("planner_sequence") or []
    if not seq or seq[0] != "inspect_verified_design":
        failures.append("tool semantics: sequence did not start with inspection")
    if not stress.get("explicit_stop_seen"):
        failures.append("STOP semantics: no Python-authorized stop")
    if not report.get("agent_success"):
        failures.append("final Python goal is not satisfied")

    req = bridge_stats["verify_requests"]
    hits = bridge_stats["verify_cache_hits"]
    misses = bridge_stats["verify_cache_misses"]
    rf = bridge_stats["rf_scenario_simulations"]
    if req != hits + misses:
        failures.append("cache accounting: requests != hits + misses")
    if rf != 3 * misses:
        failures.append("cache accounting: RF simulations != 3 * misses")

    mode = "warm" if bridge_stats["checkpoint_status"] == "hit" else "cold"
    if mode == "warm" and (misses != 0 or rf != 0 or hits != req):
        failures.append("warm accounting: expected all hits and zero RF simulations")
    if mode == "cold" and bridge_stats["checkpoint_status"] != "rebuilt":
        failures.append("cold accounting: checkpoint was not rebuilt")
    ...
    state_path = Path(cache_dir) / "regression_last_cold_v43_p3.json"
    if mode == "cold" and not failures:
        state_path.write_text(json.dumps(jsonable(signature), indent=2), encoding="utf-8")
    elif mode == "warm":
        if not state_path.is_file():
            failures.append("cold/warm pair: no prior passing cold regression record")
        else:
            cold = json.loads(state_path.read_text(encoding="utf-8"))
            _regression_compare_map("cold/warm final design", cold["final_parameters"],
                                    signature["final_parameters"], design_atol, failures)
            for scenario in ("nominal", "Q80_Cp40", "Q60_Cp60"):
                _regression_compare_map(
                    f"cold/warm {scenario}", cold["final_metrics"][scenario],
                    signature["final_metrics"][scenario], metric_atol, failures)

    result = {
        "passed": not failures,
        "mode": mode,
        "baseline": "embedded_certified_v43" if embedded else str(baseline_path),
        "failures": failures,
    }
    print("\nV4.3-P3 REGRESSION CHECK:", "PASS" if result["passed"] else "FAIL")
    print(json.dumps(result, indent=2))
    return result


def _load_required_result_json(path_text, label):
    """Fail-fast validation used before any API call or RF computation."""
    ...
    required = ("final_parameters", "final_metrics", "stress_test_evaluation",
                "agent_success", "python_bridge")
...
def main() -> int:
    args = parse_args()
    # Regression inputs are validated before output creation, DeepSeek preflight,
    # cache mutation, or RF work.  A placeholder/typo therefore fails in <1 s.
    baseline_payload = None
    if args.regression_check and args.regression_check != "embedded":
        try:
            _, baseline_payload = _load_required_result_json(
                args.regression_check, "V4.3 regression baseline"
            )
        except (FileNotFoundError, ValueError) as exc:
            print(f"REGRESSION INPUT ERROR: {exc}")
            print("No DeepSeek request, cache change, or RF simulation was performed.")
            return 2
    if args.recover_regression_cold:
        if not args.regression_check:
            print("REGRESSION INPUT ERROR: --recover-regression-cold requires regression checking to be enabled")
            return 2
        try:
            _, completed = _load_required_result_json(
                args.recover_regression_cold, "completed P3 result"
            )
        except (FileNotFoundError, ValueError) as exc:
            print(f"REGRESSION INPUT ERROR: {exc}")
            print("No DeepSeek request, cache change, or RF simulation was performed.")
        ...
        result = run_regression_check(
            completed, completed["python_bridge"], args.regression_check,
            cache_dir, args.regression_design_atol, args.regression_metric_atol,
        )
        print("Recovery mode used the completed JSON only; RF/DeepSeek were not run.")
    ...
    deepseek_enabled = (not args.no_llm and not args.no_deepseek_planner)
    if args.deepseek_api_diagnostic:
        ok = run_deepseek_api_diagnostic(
            flash_model=args.deepseek_flash_model,
            pro_model=args.deepseek_pro_model,
            reasoning_effort=args.deepseek_reasoning_effort,
            strict_tools=(not args.no_deepseek_strict_tools),
            test_pro=True,
        )
        return 0 if ok else 2

    if deepseek_enabled and not args.skip_deepseek_preflight:
        ok = run_deepseek_api_diagnostic(
            flash_model=args.deepseek_flash_model,
            pro_model=args.deepseek_pro_model,
            reasoning_effort=args.deepseek_reasoning_effort,
        ...
        if not ok and not args.allow_deterministic_fallback:
            print("DeepSeek preflight failed; expensive RF optimization was NOT started.")
            return 2
        if not ok:
            print("WARNING: DeepSeek preflight failed; deterministic fallback is explicitly allowed.")
    ...
    # V4.3 persistent Python engineering bridge.
    global _V43_BRIDGE
    _V43_BRIDGE = PersistentEngineeringBridge(
        Path(args.cache_dir), enabled=(not args.no_cache)
    )
    if args.cold_rebuild:
        _V43_BRIDGE.clear_verification_cache()
        args.rebuild_cache = True
    checkpoint_fp = _V43_BRIDGE.checkpoint_fingerprint(low, high, f_ghz, args.freq_points)
    cached_bundle = None if args.rebuild_cache else _V43_BRIDGE.load_checkpoint(checkpoint_fp)

    if cached_bundle is not None:
        print("V4.3 checkpoint cache: HIT — restored verified >=50-dB engineering checkpoint")
        best_label = cached_bundle["best_label"]
        selected = cached_bundle["selected"]
        selected_v = cached_bundle["selected_v"]
        recovery_final = cached_bundle["recovery_final"]
        recovery_v = cached_bundle["recovery_v"]
        recovery_history = cached_bundle["recovery_history"]
        recovery_ok = cached_bundle["recovery_ok"]
        last_history = cached_bundle["last_history"]
        ultra_start = cached_bundle["ultra_start"]
        ultra_start_v = cached_bundle["ultra_start_v"]
        ultra_df = cached_bundle["ultra_df"]
        ultra_ok = cached_bundle["ultra_ok"]
        l3_start = cached_bundle["l3_start"]
        l3_start_v = cached_bundle["l3_start_v"]
        l3_history = cached_bundle["l3_history"]
        l3_df = cached_bundle["l3_df"]
        l3_ok = cached_bundle["l3_ok"]
        last_ok = cached_bundle["last_ok"]
        final_50db = cached_bundle["final_50db"]
        final_50db_v = cached_bundle["final_50db_v"]
        ...
        print("V4.3 checkpoint cache: REBUILD — running deterministic RF checkpoint pipeline")
        ...
        recovery_final, recovery_v, recovery_history, recovery_ok = (
            final_lower_stopband_recovery(selected, low, high, f_ghz)
        )
        ...
        _V43_BRIDGE.save_checkpoint(checkpoint_fp, {
            "best_label": best_label,
            "selected": selected,
            "selected_v": selected_v,
            "recovery_final": recovery_final,
            "recovery_v": recovery_v,
            "recovery_history": recovery_history,
            "recovery_ok": bool(recovery_ok),
            "last_history": last_history,
            "ultra_start": ultra_start,
            "ultra_start_v": ultra_start_v,
            "ultra_df": ultra_df,
            "ultra_ok": bool(ultra_ok),
            "l3_start": l3_start,
            "l3_start_v": l3_start_v,
            "l3_history": l3_history,
            "l3_df": l3_df,
            "l3_ok": bool(l3_ok),
            "last_ok": bool(last_ok),
            "final_50db": final_50db,
            "final_50db_v": final_50db_v,
        })
        _V43_BRIDGE.flush()
    goal = AgentGoal(
    ...
    ripple_final, ripple_v, ripple_history, ripple_df, ripple_ok = run_engineering_agent_v4_3(
        final_50db, low, high, f_ghz, goal,
        flash_model=args.deepseek_flash_model,
        pro_model=args.deepseek_pro_model,
        reasoning_effort=args.deepseek_reasoning_effort,
        max_agent_steps=args.agent_steps,
        use_deepseek=(not args.no_llm and not args.no_deepseek_planner),
        escalation_rollbacks=args.deepseek_escalation_rollbacks,
        strict_tools=(not args.no_deepseek_strict_tools),
        enforce_planner_policy=(not args.allow_planner_policy_deviation),
    ...
    stress_eval = {
        ...
        "deepseek_tool_calls": sum(
            1 for r in ripple_history
            if (r.get("planner", {}) or {}).get("source") == "deepseek_tool_call"
        ),
        "deepseek_flash_steps": sum(
            1 for r in ripple_history
            if (r.get("planner", {}) or {}).get("planner_model") == args.deepseek_flash_model
        ),
        "deepseek_pro_steps": sum(
            1 for r in ripple_history
            if (r.get("planner", {}) or {}).get("planner_model") == args.deepseek_pro_model
        ),
        "deepseek_errors": [
            (r.get("planner", {}) or {}).get("deepseek_error")
            for r in ripple_history
            if (r.get("planner", {}) or {}).get("deepseek_error")
        ],
        "deterministic_fallback_steps": sum(
            1 for r in ripple_history
            if (r.get("planner", {}) or {}).get("source") == "deterministic_fallback"
        ),
    ...
    report = {
        ...
        "agent_version": "V4.3-P3_ParallelBridge_Cache_Regression",
        "agent_models": {"flash": args.deepseek_flash_model, "pro": args.deepseek_pro_model, "reasoning_effort": args.deepseek_reasoning_effort},
        "agent_goal": asdict(goal),
        "agent_trace": ripple_history,
        "stress_test_evaluation": stress_eval,
        "agent_success": bool(ripple_ok),
        "python_bridge": _V43_BRIDGE.state_snapshot(),
        "cache_directory": str(_V43_BRIDGE.cache_dir),
        "final_parameters": ripple_final.to_dict(),
        "final_metrics": ripple_v,
    }

    outdir.mkdir(parents=True, exist_ok=True)
    (outdir/"59_3Rx4_ENGINEERING_AGENT_V4_3_DEEPSEEK_RESULT.json").write_text(
        json.dumps(jsonable(report), indent=2),
        encoding="utf-8"
    )

    print("\n" + "="*78)
    print("3Rx4 AUTONOMOUS ENGINEERING AGENT V4.3 DEEPSEEK + PERSISTENT PYTHON BRIDGE RESULT")
    print("="*78)
    ...
    _V43_BRIDGE.flush()
    bridge_stats = _V43_BRIDGE.state_snapshot()
    print("\nV4.3-P3 PYTHON BRIDGE / CACHE STATISTICS")
    print(json.dumps(bridge_stats, indent=2))

    regression = None
    if args.regression_check:
        regression = run_regression_check(
            report, bridge_stats, args.regression_check, _V43_BRIDGE.cache_dir,
            args.regression_design_atol, args.regression_metric_atol,
        )
        report["regression_check"] = regression
        report["python_bridge"] = bridge_stats
        (outdir/"59_3Rx4_ENGINEERING_AGENT_V4_3_DEEPSEEK_RESULT.json").write_text(
    ...
    print(" ", outdir/"59_3Rx4_ENGINEERING_AGENT_V4_3_DEEPSEEK_RESULT.json")
    return 0 if regression is None or regression["passed"] else 3
