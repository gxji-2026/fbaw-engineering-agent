...
V4.1-P3 parallel cold-only build:
    P3-style multiprocessing is used only for independent local RF candidate verification.
    No warm-run restore, disk verification cache, or persistent checkpoint is used.
    Every launch is a full cold run.
...
# =============================================================================
# P3-style local parallel batch verification — COLD RUN ONLY
# =============================================================================
# This version deliberately has NO disk cache, NO warm checkpoint restore, and
# NO persistent engineering-state cache. Every program launch recomputes the RF
# candidates from scratch. Only independent candidate simulations inside a
# sweep/grid are evaluated in parallel; acceptance/scoring remains in the parent
# process and in the original deterministic order.
#
# Default worker count is conservative for a 6-core desktop: 4 processes.
# Override only when desired, e.g.:
#     set FBAW_WORKERS=6
#     python fbaw_engineering_agent_3Rx4_V4_1_P3_parallel_cold.py ...
_FBAW_WORKERS = int(os.getenv("FBAW_WORKERS", "4") or "4")
...
def _worker_verify_one(args):
    ...
    if extended:
        return verify_cellwise_extended(design, _W_LOW, _W_HIGH, _W_F)
    return verify_cellwise(design, _W_LOW, _W_HIGH, _W_F)


def _get_pool(workers, low, high, f_ghz):
    """Spawn once per run and reuse workers across independent candidate batches."""
    global _POOL, _POOL_CTX
    key = (id(low), id(high), id(f_ghz), workers)
...
def _verify_batch(designs, low, high, f_ghz, extended=False):
    """Evaluate independent RF candidates in parallel, preserving input order."""
    if not designs:
        return []

    workers = max(1, min(_FBAW_WORKERS, os.cpu_count() or 1, len(designs)))
    if workers <= 1 or len(designs) < _PARALLEL_MIN_BATCH:
        fn = verify_cellwise_extended if extended else verify_cellwise
        return [fn(d, low, high, f_ghz) for d in designs]
...
if __name__ == "__main__":
    _FBAW_RUN_T0 = time.perf_counter()
    try:
        _FBAW_EXITCODE = main()
    finally:
        _FBAW_RUN_ELAPSED_S = time.perf_counter() - _FBAW_RUN_T0
        print("\n" + "="*78)
        print("COLD RUN TIMING")
        print("="*78)
        print(f"elapsed_s   = {_FBAW_RUN_ELAPSED_S:.2f}")
        print(f"elapsed_min = {_FBAW_RUN_ELAPSED_S/60.0:.3f}")
        print(f"workers     = {_FBAW_WORKERS}")
    raise SystemExit(_FBAW_EXITCODE)
